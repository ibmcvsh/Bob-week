...
1. **Lab 1**: Migrate from WebSphere to Liberty
2. **Lab 2**: Upgrade from Java 8 to Java 21
3. **Lab 3**: Modernize UI from Struts to Angular
4. **Lab 4**: Generate comprehensive unit tests
5. **Lab Alt-4**: Test-Driven Development (TDD) approach
6. **Lab 5**: Fix security vulnerabilities
...
We have pre-installed dependencies using automated scripts:
- [MacOS](install-dependencies-mac.sh) - For macOS
- [Windows](install-dependencies-windows.ps1) - For Windows
...
If your dependencies are not installed, you can manually install them:
**[Manual Installation](#manual-dependency-installation)**

**Manual Verification:**
After installing dependenices, verify installation:
```bash
java -version
mvn --version
...
**Note:** You also need **IBM Bob** installed.
...
Use our automated scripts to install all dependencies at once:

**For macOS/Linux:**
```bash
cd labs/java-mod-labs
...
- Java 8 and Java 21
- Maven 3.6+
- Node.js 18+ (optional, prompted during installation)
- Angular CLI (optional, prompted during installation)

### Manual Installation (Alternative)

If you prefer manual installation, see the detailed instructions [manual dependency installation](#manual-dependency-installation) below.
...
If the automated scripts don't work or you prefer manual installation:
...
**For detailed instructions:** https://sdkman.io/install/
...
**Important:** When you run `mvn --version`, it shows the Java version that Maven will use to build your project. This may differ from your system's default Java version. Maven uses the Java version specified by the `JAVA_HOME` environment variable or the Java installation that SDKMAN! has configured.
